...
export const load = async(): Promise<boolean> => {
  if (!loaded) {
    loaded = true;
    await fetch('https://raw.githubusercontent.com/sebastian-meier/chi23-human-ai-demo/raw/main/iterative-ui/public/data/questions.txt')
      .then((response) => {
    ...
    await fetch('https://raw.githubusercontent.com/sebastian-meier/chi23-human-ai-demo/raw/main/iterative-ui/public/data/embeddings.csv')
      .then((response) => {
        return response.text();
      })
      .then((response) => {
